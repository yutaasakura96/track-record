 ...
 */
import {
  pgTable, pgEnum, text, integer, bigint, boolean, timestamp, date, jsonb, doublePrecision,
...
export const sourceDocumentVersions = pgTable("source_document_versions", {
  ...
  /** Progress, reported by `GET /api/imports/:id`. Counts only — never content. */
  chunksTotal: integer("chunks_total").notNull().default(0),
  chunksDone: integer("chunks_done").notNull().default(0),
  /**
   * Candidates whose `quote` was not found verbatim in `extracted_text`.
   * A COUNT and never content — the author sees that the guard fired, not what
   * it caught (docs/03 §5, docs/07 §5).
   */
   ...
   */
  changedRegionShare: doublePrecision("changed_region_share"),
...
/**
 * One row per chunk of changed text an import must extract from. The row IS the
 * durable checkpoint: a chunk that succeeded is never re-sent to the model, so a
 * retry resumes at the first failed chunk rather than at chunk 1 (docs/11 §2.7).
 *
 * `start_offset`/`end_offset` index into the version's `extracted_text`. The
 * chunk body is never stored here and never crosses a workflow step boundary —
 * steps pass IDs, never text (docs/03 §5).
 */
export const importChunks = pgTable("import_chunks", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  sourceDocumentVersionId: text("source_document_version_id").notNull()
    .references(() => sourceDocumentVersions.id, { onDelete: "cascade" }),
  ...
  index("import_chunks_user_idx").on(t.userId),
]);
...
export const renderProposals = pgTable("render_proposals", {
  ...
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  renderId: text("render_id").notNull().references(() => renders.id, { onDelete: "cascade" }),
  /** `{ sections: [] }` until generation lands. See `generation_status`. */
  content: jsonb("content").notNull(),
  status: proposalStatus("status").notNull().default("pending"),
  /**
   * The proposal row exists from the moment generation starts, because
   * `POST /api/renders/:kind/generate` returns 202 with a resource to poll.
   * This column is what the poll reads.
   */
  generationStatus: generationStatus("generation_status").notNull().default("generating"),
  /**
   * A stated reason, populated when `failed`. A failure NEVER destroys or
   * mutates a stored version — the current version stays byte-identical and
